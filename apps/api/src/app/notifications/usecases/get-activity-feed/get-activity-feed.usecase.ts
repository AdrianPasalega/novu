...
import { NotificationFeedItem, NotificationRepository, SubscriberRepository } from '@novu/dal';
...
export class GetActivityFeed {
  ...
  async execute(command: GetActivityFeedCommand): Promise<ActivitiesResponseDto> {
    ...
    const notifications: NotificationFeedItem[] = await this.getFeedNotifications(command, subscriberIds);
  ...
  private async findSubscribers(command: GetActivityFeedCommand): Promise<string[]> {
    return await this.subscribersRepository.searchSubscribersDatabaseIds(
